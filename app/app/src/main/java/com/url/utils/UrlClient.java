...
public class UrlClient {
    ...
    com.url.utils.RetryManager retryManager;
     ...
     */
    public static UrlClient create() {
     ...
     * <br>
     * The value can be overwritten by calling {@link UrlClient#setDefaultHeader(String, Object)} and/or
     * {@link com.url.utils.Request#header(String, Object)}.
     * <br>
     ...
     * @see com.url.utils.Request#header(String, Object)
     */
     ...
     * the URIs relatively. The value is only taken when {@link UrlClient#setBaseUri(String)} is not called or
    ...
    public static void setGlobalBaseUri(String globalBaseUri) {
        UrlClient.globalBaseUri = globalBaseUri;
    }
    ...
    public static void setJsonIndentFactor(int indentFactor) {
        UrlClient.jsonIndentFactor = indentFactor;
    }
     ...
     * timeout is odd.<br>
     * Can be overwritten for each Request with {@link com.url.utils.Request#connectTimeout(int)}.
     ...
     * timeout is odd.<br>
     * Can be overwritten for each Request with {@link com.url.utils.Request#readTimeout(int)}.
     ...
     * <br>
     * The value takes precedence over {@link UrlClient#setGlobalHeader(String, Object)} but can be overwritten by
     * {@link com.url.utils.Request#header(String, Object)}.
     * <br>
     * For the supported types for values see {@link Request#header(String, Object)}.
     *
    ...
    /**
     * Registers an alternative {@link com.url.utils.RetryManager}.
     * @param retryManager the new manager for deciding whether it makes sense to retry a request.
     */
    public void setRetryManager(com.url.utils.RetryManager retryManager) {
    ...
    public Request get(String pathOrUri) {
        return new Request(this, com.url.utils.Request.Method.GET, buildPath(pathOrUri));
    }
    ...
    public Request post(String pathOrUri) {
        return new Request(this, com.url.utils.Request.Method.POST, buildPath(pathOrUri));
    }
    ...
    public Request put(String pathOrUri) {
        return new Request(this, com.url.utils.Request.Method.PUT, buildPath(pathOrUri));
    }
    ...
    public Request delete(String pathOrUri) {
        return new Request(this, com.url.utils.Request.Method.DELETE, buildPath(pathOrUri));
    }
    ...
    <T> com.url.utils.Response<T> execute(Request request, Class<T> clazz) {
        com.url.utils.Response<T> response = null;
        ...
        if (request.retryCount == 0) {
            ...
            if (retryManager == null) {
                retryManager = com.url.utils.RetryManager.DEFAULT;
            }
            for (int tries = 0; tries <= request.retryCount; ++tries) {
                ...
                } catch (com.url.utils.WebbException we) {
    ...
    private <T> com.url.utils.Response<T> _execute(Request request, Class<T> clazz) {
        com.url.utils.Response<T> response = new com.url.utils.Response<T>(request);
        ...
        try {
            ...
            if (request.method == com.url.utils.Request.Method.GET &&
                    !uri.contains("?") &&
                    request.params != null &&
                    !request.params.isEmpty()) {
                uri += "?" + com.url.utils.WebbUtils.queryString(request.params);
            }
            ...
            com.url.utils.WebbUtils.addRequestProperties(connection, mergeHeaders(request.headers));
            if (clazz == JSONObject.class || clazz == JSONArray.class) {
                com.url.utils.WebbUtils.ensureRequestProperty(connection, HDR_ACCEPT, APP_JSON);
            }

            if (request.method != com.url.utils.Request.Method.GET && request.method != com.url.utils.Request.Method.DELETE) {
                if (request.streamPayload) {
                    com.url.utils.WebbUtils.setContentTypeAndLengthForStreaming(connection, request, request.compress);
                    connection.setDoOutput(true);
                    streamBody(connection, request.payload, request.compress);
                } else {
                    byte[] requestBody = com.url.utils.WebbUtils.getPayloadAsBytesAndSetContentType(
            ...
            is = com.url.utils.WebbUtils.wrapStream(connection.getContentEncoding(), is);

            if (clazz == InputStream.class) {
                is = new AutoDisconnectInputStream(connection, is);
            }
            if (response.isSuccess()) {
                com.url.utils.WebbUtils.parseResponseBody(clazz, response, is);
            } else {
                com.url.utils.WebbUtils.parseErrorResponse(clazz, response, is);
            }
            if (clazz == InputStream.class) {
                closeStream = false;
            }
        ...
        } catch (com.url.utils.WebbException e) {

            throw e;

        } catch (Exception e) {

            throw new com.url.utils.WebbException(e);
    ...
    private void streamBody(HttpURLConnection connection, Object body, boolean compress) throws IOException {
        ...
        try {
            ...
            com.url.utils.WebbUtils.copyStream(is, os);
            os.flush();
        } finally {
            if (os != null) {
                try { os.close(); } catch (Exception ignored) {}
